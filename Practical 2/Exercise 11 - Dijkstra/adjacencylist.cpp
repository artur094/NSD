...
Graph* graph_init(long number_nodes, long number_edges, long offset){
    ...
    graph->neighbours_weight = new int[number_edges];
    ...
    memset(graph->neighbours_weight, 0, sizeof(int)*number_nodes);
...
void graph_deinit(Graph* graph){
    ...
    delete [] graph->neighbours_weight;
 ...
 * @return
 */
int graph_add_edge(Graph* graph, long node, long neighbour, int weight){
    ...
    for(int i=first_neighbour;i<graph->graph_degree[index]+first_neighbour;i++){
        ...
        if(graph->neighbours[i] < 0) {
            ...
            graph->neighbours_weight[i] = weight;
            return TRUE;
        }
        ...
        if(graph->neighbours[i] > neighbour){
            swap_long(graph->neighbours[i], neighbour);
            swap_int(graph->neighbours_weight[i], weight);
        }
    }
    ...
    for(int i=0;i<graph->number_nodes;i++){
        ...
        for(int j=first_neighbour;j<graph->graph_degree[i]+first_neighbour;j++){
            if(node < graph->neighbours[j])
                cout << (node+graph->offset) << " " << (graph->neighbours[j]+graph->offset) << " " << graph->neighbours_weight[j] <<endl;
        }
    }
}
...
int graph_load_data(Graph* graph, fstream &file){
    ...
    while(file>>nodeA>>nodeB>>weight){
        ...
        graph_add_edge(graph, nodeA, nodeB, weight);
        graph_add_edge(graph, nodeB, nodeA, weight);
    }
