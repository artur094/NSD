...
The proposed exercise is able to find the number of nodes by taking the minimum and maximum ID in the file.
In this way, it works with both files starting from ID = 0 or ID = 1 (or even different).
Then this solution consider every node between the minimum ID and maximum ID that are not in the file as a single component not connected with others node.
Concerning with edges, it count how many edges are on the file (how many rows).

It is possible to specify the file name as argument of the main:
    ./a.out <file_name>
...
- Friendster with 68,349,466 nodes and 2,586,147,869 edges (NOT YET)
...
2 - With 382,219 nodes and 33,115,812 edges it required 38 seconds (Actor Collaborations)
3 - still to get