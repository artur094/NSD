...
long* graph_edges_array(Graph* graph){
    long *edges = new long[graph->number_edges];
    cout << "Number of nodes: "<<graph->number_nodes<<endl;
    cout << "Number of edges: "<<graph->number_edges<<endl;
    long index = 0;
    ...
    for (int i = 0; i < graph->number_nodes; ++i) {
        for (int j = 0; j < graph->nodes[i].degree; ++j) {
            long neighbour = graph->nodes[i].neighbours.at(j);
            if(neighbour > i){
                //cout << "Adding "<<i<<" "<<neighbour<<endl;
                edges[index++] = i;
                edges[index++] = neighbour;
            }
